...
console.log('JS는 홈따옴표도 문자열로 취급함');
//console.log(문자열);
...
// 변수종류 변수명;
var namber1;
console.log(namber1); // undefined

// 초기화 ( ==변수에 처음 값을 할당하는 과정을 초기화 라고 함 )
// 선언된 변수에 값을 처음 대입하는것
// [작성법]
// 변수명 = 값;
number1 = 10;
...
// 2) 괄호 내부에 + 말고 ,를 작성해 각각의 값을 독립적으로 출력할수 있음
// 형변환 없이 출력되어 문자열 따로, 다른 데이터타입 따로 출력됨
console.log("number1 : ", number1, "/ number2 : ", namber2);
...
//같은 이름으로 선언을 또다시 하려고 해서 에러남

number3 = 500; // 기존 변수 number3에 새로운 값을 덮어쓰기(제대입)
...
// 상수를 나타내는 값은 대문자로 지정함
// 중복 선언 불가
// 선언만 할 수 없음
// 선언 및 초기화가 동시에 이루어져야 한다
const PI = 3.141592;
//const PI;
//const test;


PI = 1.23; // TypeError: assignt to constant variable.
...
//console.log(num1); //ReferenceError: number2 is not defined
//console.log(num2); //ReferenceError: number2 is not defined
...
// 가능... 왜? 함수레벨 스코프인 var 는 블록을 무시하기 때문에 가능하다!
...
example(); // 함수의 호츌. 호출을 해야 함수가 수행되면서 함수내에 작성된 코드가 실행됨.
//선언과 호출 세트.. 선언을 했으면 호출을 반드시 해라!!!!!!!!!!!!!

console.log(test3); // 
// var는 블록레벨만 무시할뿐, 함수레벨은 무시하지 않는다
// (==함수 외부에서는 사용 불가)
...
// typeof 연산자 : 변수/값의 자료형을 출력해두는 연산자
...
console.log("isTrue : ", isFalse, typeof isTrue);
console.log("isTrue : ", isFalse, typeof isFalse);
...
console.log("numbers 배열 값 중 0번 쨰 :" ,number[0], typeof numbers[0])
console.log("numbers 배열 값 중 1번 쨰 :" ,number[1], typeof numbers[1])
console.log("numbers 배열 값 중 0번 쨰 :" ,number[2], typeof numbers[2])
// console.log("numbers 배열 값 중 0번 쨰 :" number[3], typeof numbers[3])
// undefined - numbers[3] 에는 아무 값도 없어서 undefined가 되었다
...
// (id, pw, userName)k 가됨 (user01, pass01, 홍길동) v가 됨
...
// const 변수명 = function() {};
// function 뒤에 괄호를 붙이는데
// 이것은 이름을 모른다고 해서 익명함수라 지정
// 변수명 == 함수명

const sumFn = function (a, b) { return a + b; };
// (a, b)를 넣은 소괄호의 정체는 바로 더하기 이다
// 그래서 숫자 a+b이 나온다 const sumFn = function(a+b)
// 즉 해당 식은 parameter를 리턴 한다면 더한다는 뜻이다

// 함수를 지정할때 사용(전달받은)하는 변수 : 매개변수(parameter)
...
// 함수를 호출할때 전달하는 값이라고 해서 전달인자 함 (Argument)
